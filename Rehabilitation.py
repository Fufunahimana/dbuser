...
class Rehabilit(Frame):
    def __init__(self):
        ...
        self.lbl_DatReh = Label(AttrFrame, text="Date Réhabilitation :", font=("times new roman",10), bg="white")
        self.lbl_DatReh.grid(row=0, column=0, sticky=W, padx=5, pady=2)

        self.ecri_DatReh = DateEntry(AttrFrame, font=("times new roman", 10), bg="lightyellow", state="readonly", locale='fr_FR',date_pattern = 'dd/mm/yyyy')
        self.ecri_DatReh.grid(row=0, column=1, sticky=W, padx=5, pady=2)

        self.lbl_Rmq = Label(AttrFrame, text="Remarque :", font=("times new roman",12),bg="white")
        self.lbl_Rmq.grid(row=1, column=1, sticky=W, padx=5, pady=2)
